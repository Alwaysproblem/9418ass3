...
path_t = './train'
path_test = './conll_test_features'

# np.random.seed(1)

def Load_test_Data(A, Comp_dims, x_file_path, sample = None, replace = False):
    x_comp_data = []
    # y_label = []
    # name = path.split("_")[-1]
    print(f"Loading {x_file_path} Data...")
    ...
    # print(f"Parsing {x_file_list} xfiles.")
    ...
    with open(x_file_path, "r") as xf:
        compX = []
        content = [row.strip() for row in xf.readlines()]
        for ind in range(1, int(content[-1].split(" ")[0]) + 1):
    ...
    return np.concatenate(x_comp_data, axis = 0)
...
def SVGP(X, y, X_test, y_test, C_num, start = 1):
    ...
    # return pred + start
    return train_acc, test_acc, SVGP

def SVGPModel(X, y, C_num, start = 1):
    """
    the X should like: (batch_size, dims)
    the y should like: (batch_size, 1) and start with 0 not 1
    """
...
def Test_File(x_file_list, SVGP_model, A, Comp_dims):
...
def main():
    ...
    X_train, y_train = Load_Data(A, Comp_dims, path=path_t)
    # X_dev, y_dev = Load_Data(A, Comp_dims, path=dev_path, sample=0.1)

    # x_file_path = './conll_test_features'


    # print(y_dev)

    SVGP_model = SVGPModel(X_train, y_train, C, 0)
    ...
    dir = os.path.dirname(__file__)
    x_file_list = glob.glob(os.path.join(dir, path_test + "/*.x"))
    x_file_list.sort(key=lambda i: len(i))
    
    # X_test = Load_test_Data(A, Comp_dims,x_file_list[0])
    # print(X_test.shape)
    Test_File(x_file_list, SVGP_model, A, Comp_dims)
